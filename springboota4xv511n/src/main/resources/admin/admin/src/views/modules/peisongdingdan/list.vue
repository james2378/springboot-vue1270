<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"20px 20px 20px","overflow":"hidden","color":"#93a2a9","background":"#e9ecf3","fontSize":"14px","position":"relative"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #ddd","padding":"20px 20px 0","margin":"0px 0 0","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 0 10px","borderRadius":"0px","textAlign":"center","flexWrap":"wrap","background":"#fff","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","order":"2"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"flex"}' class="select" label="配送接单" prop="peisongjiedan">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">配送接单</label>
						<el-select clearable v-model="searchForm.peisongjiedan" placeholder="请选择配送接单" >
							<el-option v-for="(item,index) in peisongjiedanOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"0px","margin":"0 0 10px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","background":"none","display":"flex","width":"auto","fontSize":"12px"}'>
					<el-button class="add" v-if="isAuth('peisongdingdan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('peisongdingdan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #bababa","padding":"0","margin":"0 0 0 0","borderRadius":"2px","background":"rgba(255,255,255,1)","width":"calc(100% - 0px)","float":"right"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="shouhuorenlianxifangshi"
						label="收货人联系方式">
						<template slot-scope="scope">
							{{scope.row.shouhuorenlianxifangshi}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="beizhu"
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="备货时间">
						<template slot-scope="scope">
							{{scope.row.beihuoshijian}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('peisongdingdan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"inherit","color":"#3bb4f2","display":"inline-block"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('peisongdingdan','接单')" @click="peisongjiedanCrossAddOrUpdateHandler(scope.row,'cross','','','peisongjiedan','已接单','已接单,未接单'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
								接单
							</el-button>
							<el-button class="edit" v-if=" isAuth('peisongdingdan','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('peisongdingdan','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<peisongjiedan-cross-add-or-update v-if="peisongjiedanCrossAddOrUpdateFlag" :parent="this" ref="peisongjiedanCrossaddOrUpdate"></peisongjiedan-cross-add-or-update>
...
import peisongjiedanCrossAddOrUpdate from "../peisongjiedan/add-or-update";
	export default {
		data() {
			return {
				...
				peisongjiedanCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			peisongjiedanCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.peisongjiedanCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.peisongjiedanCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.peisongjiedanCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.peisongjiedan!='' && this.searchForm.peisongjiedan!=undefined){
            params['peisongjiedan'] = this.searchForm.peisongjiedan
          }
    params['sort'] = 'beihuoshijian';
    params['order'] = 'desc';
			this.$http({
	...
	peisongdingdanstatusChange(e,row){
		if(row.status==0){
